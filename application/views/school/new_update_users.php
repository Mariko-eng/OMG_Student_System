                ...
                <div class="card-body">
                    <?= form_open_multipart('' ,'class="needs-validation"') ?>
                        ...
                        <div class="form-group row">
                            ...
                            placeholder="email"
                            value="<?= set_value('email', isset($row) ? $row['emial'] : '') ?>">
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <select required <?= $subtitle == 'view' ? 'disabled' : '' ?> class="col-md-10" name="group[]" id="group" multiple>
                                <option value="non_admin" <?= set_select('group', 'A', 
                                isset($row) && isset($row['group']) && in_array('A', $row['group']) ? true : '')  ?>>
                                Regular User</option>
                                <option value="admin" <?= set_select('group', 'B', 
                                isset($row) && isset($row['group']) && in_array('B', $row['group']) ? true : '')  ?>>
